	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Banner']))
		{
			...
			if(!empty($gambar_upload))
			{
				...
				if($model->save())
				{
					$path = Yii::getPathOfAlias('webroot').'/banner/temp/'.$model->gambar;
	                $gambar_upload->saveAs($path);

					$img = Yii::app()->simpleImage->load($path);
					$img->resizeToWidth(583);
					$img->resizeToHeight(280);
					$img->save(Yii::getPathOfAlias('webroot').'/banner/'.$model->gambar);
